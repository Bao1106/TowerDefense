...
namespace BuildReportTool
{
    ...
    public class AssetList
    {
        ...
        [SerializeField]
        private SizePart[] _all;

        private int[] _viewOffsets;

        [SerializeField]
        private SizePart[][] _perCategory;
        ...
        public SizePart[][] PerCategory => _perCategory;
        ...
        public SizePart[] TopLargest => _topLargest;
        ...
        private SortType _currentSortType = SortType.RawSize;
        ...
        private static void SortRawSize(SizePart[] assetList, SortOrder sortOrder)
        {
        ...
        private static void SortImportedSizeOrRawSize(SizePart[] assetList, SortOrder sortOrder)
        {
            if (sortOrder == SortOrder.Descending)
            {
                Array.Sort(assetList, delegate(SizePart entry1, SizePart entry2)
                {
                    if (entry1.ImportedSizeOrRawSize > entry2.ImportedSizeOrRawSize)
                    {
                        return -1;
                    }
                    if (entry1.ImportedSizeOrRawSize < entry2.ImportedSizeOrRawSize)
        ...
        private static void SortImportedSize(SizePart[] assetList, SortOrder sortOrder)
        {
            if (sortOrder == SortOrder.Descending)
            {
                Array.Sort(assetList, delegate(SizePart entry1, SizePart entry2)
                {
                    if (entry1.ImportedSizeBytes > entry2.ImportedSizeBytes)
                    {
                        return -1;
                    }
                    if (entry1.ImportedSizeBytes < entry2.ImportedSizeBytes)
        ...
        private static void SortSizeBeforeBuild(SizePart[] assetList, SortOrder sortOrder)
        {
            if (sortOrder == SortOrder.Descending)
            {
                Array.Sort(assetList, delegate(SizePart entry1, SizePart entry2)
                {
                    if (entry1.SizeInAssetsFolderBytes > entry2.SizeInAssetsFolderBytes)
                    {
                        return -1;
                    }
                    if (entry1.SizeInAssetsFolderBytes < entry2.SizeInAssetsFolderBytes)
        ...
        private static void SortPercentSize(SizePart[] assetList, SortOrder sortOrder)
        {
            if (sortOrder == SortOrder.Descending)
            {
                Array.Sort(assetList, delegate(SizePart entry1, SizePart entry2)
        ...
        private static void SortAssetFullPath(SizePart[] assetList, SortOrder sortOrder)
        {
            if (sortOrder == SortOrder.Descending)
            {
                Array.Sort(assetList, delegate(SizePart entry1, SizePart entry2)
        ...
        private static void SortAssetName(SizePart[] assetList, SortOrder sortOrder)
        {
            if (sortOrder == SortOrder.Descending)
            {
        ...
        {
            long size = -1;
            string projectPath = Util.GetProjectPath(Application.dataPath);
            for (int n = 0, len = _all.Length; n < len; ++n)
            {
                string assetImportedPath = projectPath + Util.MyHtmlDecode(_all[n].Name);

                size = Util.GetFileSizeInBytes(assetImportedPath);
        ...
        public void Init(SizePart[] all, SizePart[][] perCategory, int numberOfTop, FileFilterGroup fileFilters)
        {
            All = all;
            PostSetListAll(numberOfTop);
        ...
        public void Init(SizePart[] all, SizePart[][] perCategory, int numberOfTop, FileFilterGroup fileFilters, SortType newSortType, SortOrder newSortOrder)
        {
            _currentSortType = newSortType;
        ...
        [SerializeField]
        private Dictionary<string, SizePart> _selectedForSum = new Dictionary<string, SizePart>();
        ...
        {
            double total = 0;
            foreach (KeyValuePair<string, SizePart> pair in _selectedForSum)
            {
        ...
        public bool AtLeastOneSelectedForSum => _selectedForSum.Count > 0;

        public bool IsNothingSelected => _selectedForSum.Count <= 0;
