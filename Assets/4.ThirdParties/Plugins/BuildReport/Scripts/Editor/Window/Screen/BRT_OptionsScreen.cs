...
namespace BuildReportTool.Window.Screen
{
    public class Options : BaseScreen
    {
        ...
        private string[] _saveTypeLabels = null;


        private string[] _fileFilterToUseType = new string[]
        {
        ...
        private string[] _calculationTypeLabels = new string[]
        {
            Labels.CALCULATION_LEVEL_FULL_NAME, Labels.CALCULATION_LEVEL_NO_PREFAB_NAME, Labels.CALCULATION_LEVEL_NO_UNUSED_NAME, Labels.CALCULATION_LEVEL_MINIMAL_NAME
        };

        private int _selectedCalculationLevelIdx = 0;
        ...
        public override void DrawGUI(Rect position, BuildInfo buildReportToDisplay)
        {
            ...
            {
                GUILayout.BeginHorizontal(Settings.BOXED_LABEL_STYLE_NAME);
            ...
            BuildReportTool.Options.CollectBuildInfo = GUILayout.Toggle(BuildReportTool.Options.CollectBuildInfo, "Automatically generate and save a Build Report file after building (does not include batchmode builds)");
            GUILayout.BeginHorizontal();
            GUILayout.Space(20);
            GUILayout.Label("Note: For batchmode builds, to create build reports, call BuildReportTool.ReportGenerator.CreateReport() after BuildPipeline.BuildPlayer() in your build scripts. The Build Report is automatically saved as an XML file.", Settings.BOXED_LABEL_STYLE_NAME, GUILayout.MaxWidth(525));
            GUILayout.EndHorizontal();
            GUILayout.Space(10);

            BuildReportTool.Options.AutoShowWindowAfterNormalBuild = GUILayout.Toggle(BuildReportTool.Options.AutoShowWindowAfterNormalBuild, "Automatically show Build Report Window after building (if it is not open yet)");

            BuildReportTool.Options.AutoResortAssetsWhenUnityEditorRegainsFocus = GUILayout.Toggle(BuildReportTool.Options.AutoResortAssetsWhenUnityEditorRegainsFocus, "Re-sort assets whenever the Unity Editor regains focus");
            ...
            BuildReportTool.Options.UseThreadedReportGeneration = GUILayout.Toggle(BuildReportTool.Options.UseThreadedReportGeneration, "When generating Build Reports, use a separate thread");
            GUILayout.BeginHorizontal(GUILayout.ExpandWidth(false));
            GUILayout.Space(20);
            GUILayout.Label("Note: For batchmode builds, report generation with BuildReportTool.ReportGenerator.CreateReport() is always non-threaded.", Settings.BOXED_LABEL_STYLE_NAME, GUILayout.MaxWidth(525));
            GUILayout.EndHorizontal();
            GUILayout.Space(10);

            BuildReportTool.Options.UseThreadedFileLoading = GUILayout.Toggle(BuildReportTool.Options.UseThreadedFileLoading, "When loading Build Report files, use a separate thread");
            ...
            GUILayout.Label("Data to include in the Build Report", Settings.INFO_TITLE_STYLE_NAME);
            ...
            BuildReportTool.Options.GetSizeBeforeBuildForUsedAssets = GUILayout.Toggle(BuildReportTool.Options.GetSizeBeforeBuildForUsedAssets, "Get size-before-build of Used Assets upon creation of a build report");
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label(Labels.EDITOR_LOG_LABEL + Util.EditorLogPathOverrideMessage + ": " + Util.UsedEditorLogPath);
            if (GUILayout.Button(OPEN_IN_FILE_BROWSER_OS_SPECIFIC_LABEL) && Util.UsedEditorLogExists)
            {
            ...
            {
                string filepath = EditorUtility.OpenFilePanel("", // title
                                                              "", // default path
            ...
            numberOfTopUsedInput = Regex.Replace(numberOfTopUsedInput, @"[^0-9]", "");
            // positive numbers only, no fractions
            ...
            numberOfTopUnusedInput = Regex.Replace(numberOfTopUnusedInput, @"[^0-9]", "");
            // positive numbers only, no fractions
            ...
            entriesPerBatchInput = Regex.Replace(entriesPerBatchInput, @"[^0-9]", "");
            // positive numbers only, no fractions
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label(Labels.FILTER_GROUP_FILE_PATH_LABEL + FiltersUsed.GetProperFileFilterGroupToUseFilePath());
            // display path to used file filter
