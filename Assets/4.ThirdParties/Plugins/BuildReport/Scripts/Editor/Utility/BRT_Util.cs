...
namespace BuildReportTool
{
    public static class Util
    {
        ...
        public static string GetBuildSizePathDescription(BuildInfo buildReport)
        {
            ...
            BuildPlatform buildPlatform = ReportGenerator.GetBuildPlatformFromString(buildReport.BuildType, buildReport.BuildTargetUsed);
            ...
            if (buildPlatform == BuildPlatform.Windows32 || buildPlatform == BuildPlatform.Windows64 || buildPlatform == BuildPlatform.Linux32 || buildPlatform == BuildPlatform.Linux64)
            {
                ...
                string exeFile = Path.GetFileName(buildReport.BuildFilePath);
                string dataFolder = ReplaceFileType(exeFile, "_Data");
                string buildParentFolder = GetPathParentFolder(buildReport.BuildFilePath);
            ...
            if (buildPlatform == BuildPlatform.LinuxUniversal)
            {
                ...
                string exe32File = Path.GetFileName(buildReport.BuildFilePath);
                string exe64File = ReplaceFileType(exe32File, ".x86_64");
                string dataFolder = ReplaceFileType(exe32File, "_Data");
                string buildParentFolder = GetPathParentFolder(buildReport.BuildFilePath);
        ...
        {
            return IsFileOfType(file, ".anim") || IsFileOfType(file, ".controller") || IsFileOfType(file, ".mask");
        }

        public static bool IsFileAUnityAsset(string file)
        {
            return IsFileOfType(file, ".unity") || IsFileOfType(file, ".prefab") || IsFileOfType(file, ".asset") || IsFileOfType(file, ".controller") || IsFileOfType(file, ".mat") || IsFileOfType(file, ".flare") || IsFileOfType(file, ".physicMaterial") || IsFileOfType(file, ".guiskin") || IsFileAUnityAnimation(file);
        }
        ...
        public static string GetPackageFileContents(string filename)
        {
            ...
            string defaultBuildReportToolFullPath = Application.dataPath + "/" + Options.BUILD_REPORT_TOOL_DEFAULT_FOLDER_NAME;
		...
		Debug.LogWarning(BuildReportTool.Options.BUILD_REPORT_PACKAGE_MOVED_MSG);
            ...
            string folderPath = FindAssetFolder(Application.dataPath, Options.BUILD_REPORT_TOOL_DEFAULT_FOLDER_NAME);
        ...
        public static bool ShowFileDeleteProgress(int deletedSoFar, int totalToDelete, string filepath, bool showRecoverableMsg)
        {
            ...
            if (EditorUtility.DisplayCancelableProgressBar("Deleting file " + (deletedSoFar + 1) + " of " + totalToDelete + " (" + (totalToDelete - deletedSoFar - 1) + " left)", filepath, progress))
            {
                ...
                if (deletedSoFar > 0)
                {
                    ...
                    {
                        cancelMsg += " Those files can be recovered from your " + NameOfOSTrashFolder + ".";
                    }
                }
                else
                {
        ...
        {
            string dp = projectAssetsFolderPath;
            string[] s = dp.Split("/"[0]);
            return s[s.Length - 2];
        }
        ...
        public static bool UsedEditorLogExists => File.Exists(UsedEditorLogPath);
        ...
        public static bool AttemptGetUnityFolderMonoDLLs(bool wasWebBuild, bool wasAndroidApkBuild, string editorAppContentsPath, ApiCompatibilityLevel monoLevel, StrippingLevel codeStrippingLevel, out string path, out string higherPriorityPath)
        {
            ...
            {
                editorAppContentsPath + "/Frameworks/Mono/lib/mono", editorAppContentsPath + "/Mono/lib/mono", "/Applications/Unity/Unity.app/Contents/Frameworks/Mono/lib/mono", "C:/Program Files (x86)/Unity/Data/Mono/lib/mono", "C:/Program Files (x86)/Unity/Editor/Data/Mono/lib/mono",
#if UNITY_3_5
        ...
        public static SizePart CreateSizePartFromFile(string filename, string fileFullPath, bool getRawSize = true)
        {
            SizePart outPart = new SizePart();
...
#else
            StreamWriter write = new StreamWriter(saveFilePath, false, System.Text.Encoding.UTF8);
