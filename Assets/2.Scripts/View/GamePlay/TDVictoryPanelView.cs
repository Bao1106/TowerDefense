...
public class TDVictoryPanelView : MonoBehaviour
{
    ...
    // wait for the popup to settle
    // delay between each star
    // scale 0→1 OutBack
    // punch strength after landing
    private void Awake()
    {
    ...
    private void PlayShowAnim(int earnedStars)
    {
        ...
        if (m_PopupWindow != null)
            m_PopupWindow.localScale = Vector3.one * TDConstant.VICTORY_POPUP_SCALE_FROM;
        ...
        if (m_PopupWindow != null)
            seq.Join(m_PopupWindow.DOScale(Vector3.one, TDConstant.POPUP_SHOW_DURATION).SetEase(Ease.OutBack));
        ...
        seq.AppendInterval(TDConstant.STAR_DELAY_AFTER_POPUP);
        for (int i = 0; i < m_Stars.Count; i++)
        {
            ...
            {
                // Scale 0 → 1 OutBack
                star.transform.DOScale(Vector3.one, TDConstant.STAR_POP_DURATION)
                    .SetEase(Ease.OutBack).SetUpdate(true);
                // Fade in to gold color
                star.DOColor(COLOR_STAR_EARNED, TDConstant.STAR_POP_DURATION * 0.6f)
                    .SetEase(Ease.OutCubic).SetUpdate(true)
                    .OnComplete(() =>
                    {
                        // Small punch after landing
                        star.transform
                            .DOPunchScale(Vector3.one * TDConstant.STAR_PUNCH_STRENGTH, TDConstant.STAR_PUNCH_DURATION, 5, 0.5f)
                            .SetUpdate(true);
                    });
            });
            seq.AppendInterval(TDConstant.STAR_STAGGER);
        }
    }
